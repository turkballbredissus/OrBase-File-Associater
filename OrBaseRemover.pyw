...
class OrbaseUninstaller(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Orbase Uninstaller")
        ...
        try:
            ...
            pass
        self.frames = {}
        for F in (WelcomePage, UninstallingPage, DonePage):
            frame = F(self)
            self.frames[F] = frame
...
class WelcomePage(tk.Frame):
    def __init__(self, master):
        ...
        tk.Label(self, text="Orbase Uninstaller", font=FONT_TITLE, bg=BG, fg=FG).pack()
        tk.Label(self, text="This will remove all Orbase file associations from your system.", font=FONT_SUB, bg=BG, fg="#888", justify="center").pack(pady=(10, 30))
        tk.Label(self, text="Your .orb files will not be deleted.", font=FONT, bg=BG, fg="#4caf50").pack(pady=(0, 30))
        tk.Button(self, text="Uninstall", font=FONT, bg=ACCENT, fg="white", bd=0, padx=20, pady=8, cursor="hand2",
                  command=lambda: master.show(UninstallingPage) or master.frames[UninstallingPage].start(master)).pack()


class UninstallingPage(tk.Frame):
    def __init__(self, master):
        super().__init__(master, bg=BG)
        tk.Label(self, text="Uninstalling...", font=FONT_TITLE, bg=BG, fg=FG).pack(pady=(80, 20))
    ...
    def start(self, master):
        threading.Thread(target=self.uninstall, args=(master,), daemon=True).start()

    def uninstall(self, master):
        orbs_folder = self.get_orbs_folder()
        steps = [
            ("Removing extensions...", lambda: self.del_key(r".orb")),
            ("Removing Orbfile folder from registry...", lambda: self.del_key(r"Orbfile")),
            ("Removing .orun file type...", lambda: self.del_key(r".orun")),
            ("Removing Orunfile...", lambda: self.del_key(r"Orunfile")),
            ("Removing orb_auto_file...", lambda: self.del_key(r"orb_auto_file")),
            ("Removing orun_auto_file...", lambda: self.del_key(r"orun_auto_file")),
            ("Clearing Explorer association cache...", self.del_fileexts),
            ("Removing runner script...", lambda: self.del_runner(orbs_folder)),
            ("Removing leftover temp files...", self.del_temp),
            ("Removing Orbase settings...", lambda: self.del_hkcu(r"SOFTWARE\Orbase")),
        ]
    ...
    def get_orbs_folder(self):
        try:
            k = winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"SOFTWARE\Orbase")
            val, _ = winreg.QueryValueEx(k, "OrbsFolder")
            winreg.CloseKey(k)
            return val
        except Exception:
            return None

    def del_runner(self, orbs_folder):
        if orbs_folder:
            runner = os.path.join(orbs_folder, "orun_runner.bat")
            if os.path.exists(runner):
                os.remove(runner)

    def del_fileexts(self):
        for ext in (".orb", ".orun"):
            self.del_hkcu(r"Software\Microsoft\Windows\CurrentVersion\Explorer\FileExts" + "\\" + ext)

    def del_temp(self):
        temp = os.environ.get("TEMP")
        if not temp:
            return
        for f in glob.glob(os.path.join(temp, "orbtemp.*")):
            try:
                os.remove(f)
            except OSError:
                pass
    ...
    def del_key(self, key):
        try:
            self.del_tree(winreg.HKEY_CURRENT_USER, r"Software\Classes" + "\\" + key)
        except OSError:
            pass
        try:
            self.del_tree(winreg.HKEY_CLASSES_ROOT, key)
        except OSError:
            pass

    def del_tree(self, hive, key):
        with winreg.OpenKey(hive, key, 0, winreg.KEY_READ) as k:
            while True:
                try:
                    subkey = winreg.EnumKey(k, 0)
                    self.del_tree(hive, key + "\\" + subkey)
                except OSError:
                    break
        winreg.DeleteKey(hive, key)

    def del_hkcu(self, key):
        try:
            self.del_tree(winreg.HKEY_CURRENT_USER, key)
...
class DonePage(tk.Frame):
    def __init__(self, master):
        ...
        tk.Label(self, text="Orbase Removed", font=FONT_TITLE, bg=BG, fg=FG).pack()
        tk.Label(self, text="All file associations have been removed.\nYour .orb files are still safe.", font=FONT, bg=BG, fg="#888", justify="center").pack(pady=(10, 30))
        tk.Button(self, text="We are sorry to see you leave.", font=FONT, bg=ACCENT, fg="white", bd=0, padx=20, pady=8, cursor="hand2", command=master.destroy).pack()


if __name__ == "__main__":
    app = OrbaseUninstaller()
    app.mainloop()
